...
import config
from font import freesans20
from font import writer
...
def display_temperature_and_humidity(temperature, humidity):
    ...
    temperature_pbm = load_image('icons/temperature.pbm')
    units_pbm = load_image('icons/fahrenheit.pbm') if config.FAHRENHEIT \
        else load_image('icons/celsius.pbm')
    humidity_pbm = load_image('icons/humidity.pbm')
    percent_pbm = load_image('icons/percent.pbm')
